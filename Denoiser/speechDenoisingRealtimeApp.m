...
%% Define STFT and ISTFT
stf  = dsp.STFT(win,Overlap,FFTLength);
istf = dsp.ISTFT(win,Overlap,'WeightedOverlapAdd',false);

%% This buffer is used to read most recent 8 STFT vectors
segmentBuffer = dsp.AsyncBuffer('Capacity',NumSegments);
...
scope = timescope('TimeSpan',5,'SampleRate',Fs,...
    'YLimits',[-1 1],'NumInputPorts',3,...
    'LayoutDimensions',[3 1],'BufferLength',70e3);
scope.ChannelNames = {'Noisy Audio','Denoised Audio','Noise Gate Gain','Clean Audio'};
...
while ~isDone(noisyReader) && stepCount < numSteps
    ...
    SFFT  = stf(audio);
    SFFT  = circshift(SFFT,FFTLength/2 - 1); % center
    SFFT  = SFFT(FFTLength/2:end,:);       % make one-sided
    
    % Write STFT vector
    write(segmentBuffer,abs(SFFT).');

    % Read most recent 8 STFT vectors
    SFFT_Image = read(segmentBuffer,8,7).';
    ...
    YFFT_Full = [conj(YFFT(end-1:-1:2,:)) ; YFFT];
    YFFT_Full = circshift(YFFT_Full,-(FFTLength/2-1)); % uncenter
...
hFig = findall(0,'Name','DenoisingNet','Type','figure');
editWidgets = findall(hFig,'Type','uicontrol','style','edit');
sliderWidgets = findall(hFig,'Type','uicontrol','style','slider');
defaultParamVals = [-35 0.3 0.5];
for idx = 1:numel(sliderWidgets)
    sliderWidgets(idx).Value = defaultParamVals(idx);
    editWidgets(idx).String = num2str(defaultParamVals(idx));
end
drawnow
end